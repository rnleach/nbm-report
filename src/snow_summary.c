...
snow_sum_build_scenarios(struct SnowSum *ssum)
{
    ...
    GTree *scenarios = create_scenarios_from_pdfs(ssum->pdfs, 0.2, 0.2);
    assert(scenarios);
...
show_snow_scenarios(struct SnowSum *ssum)
{
    ...
    table_add_column(tbl,  1, Table_ColumnType_SCENARIO, "Scenario-1", "%4.1lf [%4.1lf-%4.1lf] %3.0lf", 21);
    table_add_column(tbl,  2, Table_ColumnType_SCENARIO, "Scenario-2", "%4.1lf [%4.1lf-%4.1lf] %3.0lf", 21);
