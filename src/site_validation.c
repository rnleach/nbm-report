...
/** Pair up a \ref TextBuffer with an initialization time for this locations.csv file. */
struct LocationsCSV {
 ...
 *                                        MatchedSitesRecord
 *-----------------------------------------------------------------------------------------------*/
/** Represents a row in the locations.csv file. */
...
/** State for the CSV parser parsing a locations.csv file. */
struct CSVState {
    sqlite3_stmt *stmt;
    int col;
...
create_record_from_row(sqlite3_stmt *stmt)
{
    ...
    int num_bytes = asprintf(&id, "%s", sqlite3_column_text(stmt, 0));
    Stopif(num_bytes < 0, exit(EXIT_FAILURE), "out of memory");

    num_bytes = asprintf(&file_name, "%s.csv", id);
    Stopif(num_bytes < 0, exit(EXIT_FAILURE), "out of memory");

    num_bytes = asprintf(&name, "%s", sqlite3_column_text(stmt, 1));
    Stopif(num_bytes < 0, exit(EXIT_FAILURE), "out of memory");

    num_bytes = asprintf(&state, "%s", sqlite3_column_text(stmt, 2));
    Stopif(num_bytes < 0, exit(EXIT_FAILURE), "out of memory");
...
find_exact_case_insensitive_match(sqlite3 *db, char const site[static 1])
{
    ...
    int num_bytes = asprintf(&upper_case_site, "%s", site);
    Stopif(num_bytes < 1, exit(EXIT_FAILURE), "out of memory");
    to_uppercase(upper_case_site);

    char *query = 0;
    num_bytes = asprintf(&query, "SELECT id, name, state, lat, lon FROM locations WHERE id = '%s'",
                         upper_case_site);
    Stopif(num_bytes < 1, exit(EXIT_FAILURE), "out of memory");
 ...
 *                                        SiteValidation
 *-----------------------------------------------------------------------------------------------*/
/** Internal implementaiton of SiteValidation. */
