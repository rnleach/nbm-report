 ...
 */
struct SiteValidation *site_validation_create(char const *site, time_t request_time);

/** Check to see if this validation failed.
 *
 * \returns \c true if the validation failed.
 */
bool site_validation_failed(struct SiteValidation *validation);
 ...
 * failed.
 */
void site_validation_print_failure_message(struct SiteValidation *validation);
...
void site_validation_free(struct SiteValidation **validation);
