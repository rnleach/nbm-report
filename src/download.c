...
build_download_url(char const file_name[static 1], time_t data_init_time)
{
    ...
    // NBM 5.0 started about 5/15/2026
    struct tm nbm_version_50_starts_tm = {.tm_year = 2026 - 1900, .tm_mon = 5 - 1, .tm_mday = 15, .tm_hour = 11};
    time_t nbm_version_50_starts = timegm(&nbm_version_50_starts_tm);

    if (data_init_time > nbm_version_50_starts) {
        // NBM 5.0
        sprintf(url, "%s%4d/%02d/%02d/NBM5.0/%02d/%s", base_url, year, month, day, hour,
                url_file_name);
    } else if (data_init_time > nbm_version_42_starts) {
        // NBM 4.2
